...
TE: Just to look at how turtle grew over time we can extract those that were caught at least twice and look at their growths:
```{r}
turtle_data_1 %>% group_by(Turtle_ID) %>%
  mutate(n = n(),
         date = as.Date(paste(Yr, Mo, Da, sep = "-"))) %>%
  filter(n > 1) -> turtle_data_3

ggplot(data = turtle_data_3) + 
...
TE: I show how to create a date field above. Take a look and see if that makes sense. 
...
TE: Good question - it may be a bit difficult to determine their origins accurately... 
...
# you can also histogram SCL rather than size classes. 
hist(Years_1990_1995$SCL)
...
TE: I don't think all adults leave the bay every other year... so be careful with this assumption. 
...
TE: I think this is a good place to start - getting familiar with the dataset and playing with R a little.  We should start outlining the future analyses.  Here are some ideas:
1. Try to fill in SCL (or CCL) gaps as much as possible
2. Look at size distributions (not size class distributions) over time (blocks of 5 years, like you did?). You can use ridge plot (ggridges) to show how they might have changed over time (https://cran.r-project.org/web/packages/ggridges/vignettes/introduction.html)
3. If there aren't enough sample sizes in 5-year blocks, we may extend to 6, 7, 8, ... years. But, probably not necessary.
4. Are there statistical differences in the mean of sizes (SCL/CCL) over these time periods? This requires some sort of statistical analysis - a regression analysis may be useful.
5. As you mention above, it may be good to look at how the proportion of small ones (we need to define this) changed (or not changed) over time. 
